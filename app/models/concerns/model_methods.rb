module ModelMethods
  ...
  def remaining_share_or_interest_
    if self.Corporation?
      ...
      ( self.super_entity.try(:number_of_assets) || 0 ) - ( ( self.class.where(super_entity_id: self.super_entity_id).where.not(id: self.id).sum(:my_percentage) || 0 ) + ( self.my_percentage || 0 ) )
    elsif self.LLP?
      ( self.super_entity.try(:number_of_assets) || 0 ) - ( ( self.class.where(super_entity_id: self.super_entity_id).where.not(id: self.id).sum(:my_percentage) || 0 ) + ( self.my_percentage || 0 ) )
    elsif self.Partnership?
      ( self.super_entity.try(:number_of_assets) || 0 ) - ( self.class.where(super_entity_id: self.super_entity_id).where.not(id: self.id).sum(:my_percentage) || 0 )
