class Entities::LlpController < ApplicationController
  ...
  def basic_info
    ...
    if request.get?
      ...
      @entity.type_           = MemberType.new.getLLPId
  ...
  def entity_params
    params.require(:entity).permit(:name, :address, :type_, :jurisdiction, :number_of_assets,
                                   ...
                                   :postal_address, :city, :state, :zip, :date_of_formation, :ein_or_ssn, :s_corp_status, :not_for_profit_status, :legal_ending, :honorific, :is_honorific)
  end
  ...
  def add_breadcrum
    ...
    if params[:entity_key] and @entity.present? and !@entity.new_record?
      add_breadcrumb ("<div class=\"pull-left\"><h4><a href=\"#{edit_entity_path(@entity.key)}\">Edit LLP: #{@entity.name}</a></h4></div>").html_safe
    else
