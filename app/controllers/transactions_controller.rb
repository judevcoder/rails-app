  ...
  def closing
    if request.post?
      ...
      unless params[:type] == "purchase" || @transaction_property.nil?
        flash[:success] = "Congratulations on your sale of #{Property.find(@transaction_property.property_id).name} to #{@transaction.relinquishing_seller_entity.display_name} are being transferred to your Qualified Intermediary. #{Property.find(@transaction_property.property_id).name} is now being reclassified from a Purchased Property to a Sold Property. Please proceed to the Purchase Module as you only have 45 days to identify one or more Replacement Properties to Buy. It might be a good idea to go to your Account Settings so that you can receive warning alerts by email, SMS message or both."
      end
